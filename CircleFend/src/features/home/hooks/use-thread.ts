...
export function useThread(onClose: () => void) {
    const {
        register,
        handleSubmit,
        reset,
    ...
    console.log("Form state:", errors); // Untuk cek error form
    ...
    const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files && files.length > 0) {
            ...
            setValue("image", fileArray); // Set nilai sebagai array file
            const reader = new FileReader();
            reader.onloadend = () => {
                setPreviewImage(reader.result as string); // Set pratinjau gambar
            };
            reader.readAsDataURL(fileArray[0]); // Baca file pertama
        } else {
            setPreviewImage(null);
            setValue("image", []); // Kosongkan jika tidak ada file
        }
    };
