...
const MediaGallery = ({ images, onImageClick }: { images: { id: number, url: string }[], onImageClick: (url: string) => void }) => (
...
export function ProfilePage() {
    ...
    return (
        <Box p={4} maxW="800px" mx="auto" color="white" >
                ...
                >
                    ...
                    <VStack align="start" spacing={1}>
                        ...
                        <HStack spacing={3}> {/* Mengurangi jarak antara elemen */}
                            <HStack spacing={1}>
                                <Text fontSize="sm" fontWeight="bold" color="white">{profile.following}</Text>
                                <Text fontSize="sm" color="#909090">Following</Text>
                            </HStack>
                            <HStack spacing={1}>
                                <Text fontSize="sm" fontWeight="bold" color="white">{profile.followers}</Text>
                        ...
                        <TabPanels display={'flex'} flexDirection={'row'}>
                                ...
                                {profile.threads.map(post => (
                                    ...
                                    >
                                        <HStack spacing={3} mb={3}>
                                            <Avatar size="sm" name={post.user.userName} src={post.user.profileImage} />
                                            <Text fontWeight="bold">{post.user.userName}</Text>
                                            <Text fontSize={'small'} color={'grey'}> {post.user.email}</Text>
                                            <Text fontSize={'small'} color={'grey'}>4h</Text>
                                        </HStack>

                                        <Text fontSize={'md'} color={'white'} mt={2}> {post.content}</Text>
                            ...
                            <TabPanel p={0} flex="1">
                                {mediaThreads && mediaThreads.length > 0 ? (
                                    <MediaGallery
                                        images={mediaThreads.map(thread => ({ id: thread.id, url: thread.image }))}
            ...
            {/* Modal untuk menampilkan gambar detail */}
            <Modal isOpen={isImageOpen} onClose={onImageClose} size="full"> {/* Ubah ukuran modal menjadi full */}
                <ModalOverlay />
                <ModalContent bg="black" p={0} color="white"> {/* Background hitam dan padding diatur ke 0 */}
                    <ModalCloseButton color="white" /> {/* Tutup modal dengan tombol warna putih */}
                    <ModalBody p={0}> {/* Hilangkan padding pada body modal */}
                        {selectedImage && (
                            <Image
                                src={selectedImage}
                                alt="Selected Image"
